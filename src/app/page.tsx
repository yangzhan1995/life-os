...
/* ============================================================
   Data (abbreviated for space — full content preserved)
   ============================================================ */
...
const foundationLayers: LayerData[] = [
  ...
  { id: "play", title: "游戏/玩", subtitle: "做一件事不因为它有用只因为它有意思", desc: "人在玩的时候不问意义。孩子追蝴蝶、大人打球——这些时刻不解决任何问题但人在这些时刻最不焦虑、最不空虚、最不孤独。如果你的操作系统只有「解决问题」和「接受现实」两种模式你会把自己活成一个项目。", books: "《游戏的人》赫伊津哈 | 心流理论（契克森米哈赖）", principles: ["玩是文化的基础——文明本身起源于游戏不是功利的算计", "心流是玩的高级形态：完全沉浸在当下自我暂时消失时间感消失", "玩不是工作的对立面——最有效的创造往往发生在玩的状态中", "不会玩的人向外（僵硬地执行）和向内（死板地反思）都会出问题"], why: "因为你不是一个需要持续优化的产品。玩是第三种模式——它提醒你生活不只是有用。", practice: ["每周至少做一件纯粹因为好玩而做的事——不拍照不发朋友圈不总结收获", "把工作和学习游戏化——设挑战、做尝试、允许出错", "和小孩一起玩一次——不是教他们是学他们", "问自己：我上一次纯粹因为好玩做一件事是什么时候？"], icon: RiGamepadLine },
  ...
  { id: "aesthetic", title: "审美/超越", subtitle: "理性之外的治愈", desc: "人看到壮丽的自然景观空虚暂时消失——这不是向内也不是向外。人听到一段音乐被击中不知为什么流泪。历史上真正治愈人的往往不是哲学体系和修行方法而是一首诗、一段旋律、一片夕阳。", books: "什么都别读了。去听一首曲子去公园散步去美术馆发呆。", principles: ["美不需要解释——解释了反而破坏了体验", "超越不是宗教体验是「自我感」暂时消失的感受——完全沉浸在当下", "艺术、自然、音乐、沉默——这些不是逃避是另一种形式的「知道」", "在框架的最外面留白——有些东西无法被建模但它们是真实的"], why: "因为如果你的生活可以完全被框架解释和覆盖那你的生活就太窄了。框架之外的东西——美、神秘、不可言说——不是系统的bug是生活本身。", practice: ["每天花5分钟看窗外——不是思考只是看", "每周听一首从未听过的曲子——不分析不评价", "每月去一次自然里——不要拍照不要发朋友圈", "在房间的角落留一些好看的无用之物——它们存在的唯一理由是美"], icon: RiStarLine },
]

const metaItems: MetaData[] = [
  { id: "forbidden", title: "禁止清单", subtitle: "刻在系统底层的八条绝对不做的事", items: [
    { title: "情绪高峰不做重大决定", desc: "愤怒、狂喜、恐惧、绝望时的大脑不是你的大脑。这些状态下前额叶皮层被杏仁核劫持。等至少24小时再做决定。" },
    { title: "深夜不反刍", desc: "晚上11点后脑子想的问题90%在白天看起来没那么真实。深夜是大脑的「灾难化实验室」。11点后不琢磨任何人生大事。" },
    { title: "不用向外工具处理向内问题", desc: "焦虑不是效率问题不是多做一个计划就能解决的。空虚不是目标不够大。在内心痛苦时制定更多计划只会加剧痛苦。" },
    { title: "不用向内当作逃避向外的借口", desc: "「放下」「随缘」「无为」不能用来解释为什么你不敢争取、不敢面对、不敢承担责任。" },
    { title: "不自欺", desc: "对自己说谎是所有谎言里最贵的。你跟自己说过的每一个谎迟早要还——以焦虑的形式、以空虚的形式、以失眠的形式。" },
    { title: "不因害怕别人的反应而不做该做的事", desc: "该说的话不说、该设的边界不设、该走的关系不走——这些账攒着攒着就变成对自己生活的怨恨。" },
    { title: "不比较", desc: "你拿你的幕后（24小时不间断播放）比别人的高光剪辑——这个比较从一开始就输了。" },
    { title: "不放弃身体", desc: "睡眠、运动、饮食三种底线不能同时放弃。全松的信号是你不再在意自己了——这是需要求助的信号。" },
  ], icon: RiForbidLine },
  { id: "crisis", title: "危机模式", subtitle: "崩溃时不思考照着做就行", items: [
    { title: "前三天：不决定、不死扛、不独处", desc: "前三天大脑处于应激状态前额叶功能受损。不决定；允许自己躺着、哭、什么都不做；身边要有信任的人——一个人时大脑默认最坏可能性。" },
    { title: "只做五件事", desc: "按时吃饭、按时睡觉（哪怕睡不着躺着也算）、走一段路、对一个人说真实的感受（不说「我没事」）、不借酒/药/屏幕逃避。" },
    { title: "三天后启动恢复", desc: "画出可控/不可控线；有需要哀悼的就哀悼它；找一个人帮忙（能听、不急着给建议的人）；恢复日常节奏的第一个元素。" },
  ], icon: RiAlarmWarningLine },
  { id: "rhythm", title: "日常节奏", subtitle: "早晚各5分钟每周30分钟维护", items: [
    { title: "早晨5分钟开机", desc: "不刷手机。1min身体扫描 → 2min今日预判 → 1min设定一个意图关键词 → 1min身体激活。" },
    { title: "晚间5分钟关机", desc: "不看手机。3min一件事复盘 → 1min可控/不可控清单 → 1min呼吸收尾。" },
    { title: "每周30分钟维护", desc: "5min财务 → 5min身体 → 5min关系 → 10min回顾 → 5min「不做什么」。" },
  ], icon: RiTimeLine },
  { id: "resource", title: "资源管理", subtitle: "操作系统的硬件——钱、时间、健康", items: [
    { title: "钱的三条底线", desc: "紧急金≥3-6月生活费；月花销≤收入70%；不欠消费贷——利息超过通胀率的债就是坏债。" },
    { title: "时间的三个黑洞", desc: "手机离开身边至少1小时/天；每天一段连续90分钟不被打断；烂会议能拒则拒。" },
    { title: "健康的三条最低标准", desc: "睡≥6h起床时间固定；每周动≥3次每次≥20分钟心率上去；不喝含糖饮料每天至少一顿有蔬菜。" },
  ], icon: RiCoinsLine },
  { id: "exit", title: "退出键", subtitle: "能关掉的系统才是好系统", items: [
    { title: "每天退出一次", desc: "至少有一段不用框架的时间——吃饭不看手机走路不思考。不是为了「活在当下」是为了不做任何分析。" },
    { title: "每周退出半天", desc: "不计划、不分析、不优化。可以散步发呆可以和喜欢的人浪费一个下午。" },
    { title: "每月退出一次", desc: "一次自然接触——山、海、公园、日落。不拍照不用脑就在那里。" },
  ], icon: RiShutDownLine },
  { id: "mourning", title: "哀悼/丧失", subtitle: "有些东西不能被解决只能被哀悼", items: [
    { title: "哀悼不是情绪问题", desc: "哀悼是人类特有的能力：承认失去了什么，哀悼它，然后带着缺憾继续活。不能被跳跃不能被升华。" },
    { title: "哀悼不能被「接纳」跳过", desc: "斯多葛教接受佛教教放下——但哀悼不一样。用向内框架跳过哀悼代价是某个地方开始麻木。" },
    { title: "健康的哀悼", desc: "弗洛伊德：哀悼是健康的——慢慢把情感从丧失对象上收回来。忧郁是不健康的——不承认丧失把愤怒转向自己。" },
  ], icon: RiCandleLine },
]
...
export default function Home() {
  ...
  const block = blockColors[activeSection] || "block-lilac"
    ...
    return (
      <div className="space-y-10 max-w-[800px]">
        <div className="card-white p-10">
          ...
          {item.subtitle && <p className="text-[18px] font-medium mb-1">{item.subtitle}</p>}
          {item.core && <p className="text-[18px] font-semibold mb-4">{item.core}</p>}
          <hr className="border-[var(--line)] my-5" />
          {item.question && <p className="body-lg mb-2"><strong>核心追问：</strong>{item.question}</p>}
          <p className="body-lg" style={{color: "var(--mut-fg)"}}>{item.desc}</p>
          {item.books && <div className="mt-4 p-4 rounded-xl text-[15px]" style={{background: "var(--mut)"}}>{item.books}</div>}
        </div>
        {item.concepts && <BlockCard title="核心概念" items={item.concepts} numbered />}
        {item.scenarios && <BlockCard title="使用场景" items={item.scenarios} />}
        {item.training && <BlockCard title="如何训练" items={item.training} />}
        {item.pitfalls && <BlockCard title="常见误区" items={item.pitfalls} warn />}
        {item.principles && <BlockCard title="核心原则" items={item.principles} numbered />}
        {item.why && <div className="card-white p-10"><h4 className="headline mb-3">为什么重要</h4><p className="body-lg" style={{color:"var(--mut-fg)"}}>{item.why}</p></div>}
        {item.practice && <BlockCard title="实操建议" items={item.practice} />}
        {item.items && typeof item.items[0] === "object" && (
          <div className="card-white p-10">
            <h4 className="headline mb-6">详细说明</h4>
            <div className="space-y-6">
              {item.items.map((li: any, i: number) => (
                <div key={i} className="flex gap-4">
                  <span className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 mt-0.5" style={{background:"var(--mut)"}}>{i + 1}</span>
                  <div><h5 className="text-[17px] font-semibold">{li.title}</h5><p className="body-sm mt-1" style={{color:"var(--mut-fg)"}}>{li.desc}</p></div>
                </div>
              ))}
            </div>
          </div>
  ...
  const renderContent = () => {
      ...
      <div className="space-y-16">
        <div className="text-center py-16">
          ...
          <div className="flex items-center justify-center gap-2.5 mb-4 flex-wrap">
            <span className="inline-flex items-center rounded-full bg-[var(--pri)] text-[var(--pri-fg)] px-4 py-1.5 text-[14px] font-medium">能 → 向外，去搞定</span>
            <span className="inline-flex items-center rounded-full border px-4 py-1.5 text-[14px] font-medium" style={{borderColor:"var(--line)"}}>不能 → 向内，来接住</span>
          </div>
          <p className="subhead max-w-xl mx-auto" style={{color:"var(--mut-fg)"}}>能影响一部分 → 向外做能做的，向内接住剩下的</p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {outwardTools.map(t => (
                <div key={t.id} className="card-interactive p-5" onClick={() => setActiveDetail(t.id)}>
                  ...
                  <h4 className="text-[18px] font-bold">{t.full}</h4>
                  <p className="body-sm mt-1.5" style={{color:"var(--mut-fg)"}}>{t.core}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    if (activeSection === "outward") return (
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {outwardTools.map(t => (
            <div key={t.id} className="card-interactive p-7" onClick={() => setActiveDetail(t.id)}>
              ...
              <h3 className="text-[22px] font-bold">{t.full}</h3>
              <p className="text-[17px] font-medium mt-1.5">{t.question}</p>
              <p className="body-sm mt-2" style={{color:"var(--mut-fg)"}}>{t.core}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    if (activeSection === "inward") return (
      <div>
        <p className="eyebrow mb-2">向内</p>
        <h2 className="display-lg mb-3">向内六传统</h2>
        <p className="subhead mb-10" style={{color:"var(--mut-fg)"}}>六条向内的路，控制、放下、减法、选择、修身、训练</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {inwardTools.map(t => (
            <div key={t.id} className="card-interactive p-7" onClick={() => setActiveDetail(t.id)}>
              <t.icon size={28} className="mb-3 opacity-60" />
    ...
    if (activeSection === "foundation") return (
      <div>
        <p className="eyebrow mb-2">框架地图</p>
        <h2 className="display-lg mb-3">支撑层</h2>
        <p className="subhead mb-10" style={{color:"var(--mut-fg)"}}>从身体到审美，十二层支撑整个操作系统</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {foundationLayers.map(l => (
            <div key={l.id} className="card-interactive p-5" onClick={() => setActiveDetail(l.id)}>
              <l.icon size={22} className="mb-2.5 opacity-60" />
              <h4 className="text-[16px] font-bold">{l.title}</h4>
              <p className="text-[14px] mt-1" style={{color:"var(--mut-fg)"}}>{l.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    if (activeSection === "meta") return (
      <div>
        <p className="eyebrow mb-2">系统维护</p>
        <h2 className="display-lg mb-8">操作手册</h2>
        <Tabs defaultValue="forbidden">
          <TabsList>
            {metaItems.map(m => (
              <TabsTrigger key={m.id} value={m.id}>
                <m.icon size={15} className="mr-1.5" />
                {m.title}
              </TabsTrigger>
            ))}
          </TabsList>
          {metaItems.map(m => (
            <TabsContent key={m.id} value={m.id}>
              <div className="card-white p-8 mt-0">
                <div className="flex items-center gap-3 mb-6">
                  <m.icon size={26} /><div><h3 className="headline">{m.title}</h3><p className="body-sm" style={{color:"var(--mut-fg)"}}>{m.subtitle}</p></div>
                </div>
                <div className="space-y-5">
                  {m.items.map((li, i) => (
                    <div key={i} className="flex gap-3">
                      <span className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shrink-0 mt-0.5" style={{background:"var(--mut)"}}>{i + 1}</span>
                      <div><h5 className="text-[16px] font-semibold">{li.title}</h5><p className="body-sm mt-0.5" style={{color:"var(--mut-fg)"}}>{li.desc}</p></div>
                    </div>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen flex" style={{background:"var(--bg)", color:"var(--fg)"}}>
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? "w-56" : "w-14"} transition-all duration-200 border-r shrink-0 flex flex-col h-screen sticky top-0 z-30`} style={{background:"var(--bg)", borderColor:"var(--line)"}}>
        <div className="flex items-center h-14 px-3 border-b" style={{borderColor:"var(--line)"}}>
          {sidebarOpen && <span className="font-bold text-[15px] tracking-tight">人生操作系统</span>}
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1.5 rounded-full hover:bg-[var(--mut)] shrink-0 ml-auto">
      ...
      <main className="flex-1 min-w-0">
        ...
        <footer className="py-16 text-center">
          ...
          <p className="text-[13px] mt-1 opacity-50">十层四维 · 向外四思维 · 向内六传统</p>
        </footer>
      </main>
    </div>
  )
...
function BlockCard({ title, items, numbered, warn }: { title: string; items: string[]; numbered?: boolean; warn?: boolean }) {
  return (
    <div className="card-white p-10">
      <h4 className={`headline mb-6 ${warn ? "text-[var(--dst)]" : ""}`}>{title}</h4>
      <div className="space-y-3.5">
        {items.map((t, i) => (
          <div key={i} className="flex gap-3">
            {numbered
              ? <span className="w-7 h-7 rounded-full flex items-center justify-center text-[13px] font-bold shrink-0 mt-0.5" style={{background:"var(--mut)"}}>{i + 1}</span>
              : <span className={`shrink-0 mt-1 text-[15px] ${warn ? "text-[var(--dst)]" : ""}`}>{warn ? "⚠" : "▸"}</span>
            }
            <p className="body-lg" style={{color:"var(--mut-fg)"}}>{t}</p>
          </div>
        ))}
      </div>
    </div>
